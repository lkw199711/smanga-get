...
export default class OmegaScans {
    ...
    async start() {
        ...
        for (let i = 0; i < this.meta.chapters.length; i++) {
            ...
            if (chapter.price > 0) {
                write_log(`[subscribe]${this.name} 章节 ${chapter.name} 需要付费 ${chapter.price}，跳过下载`);
                continue;
            }
            end_app(); // 结束应用
    ...
    async download_chapter(chapter: any) {
        ...
        const chapterFolder = `${this.mangaFolder}/${chapter.name}`;
        if (!fs.existsSync(chapterFolder)) {
            await fs.promises.mkdir(chapterFolder, { recursive: true });
        } else {
            // 如果章节文件夹已存在，检查是否需要重新下载
            const existingImages = fs.readdirSync(chapterFolder).filter(file => file.endsWith('.jpg'));
...
*/
        ...
        const [res, error] = await this.page.goto(chapterUrl, {
        ...
        const imageUrls = imagesHtml.match(/<img[^>]+/gs)?.map((img: string) => img.match(/src="([^"]+)"/)?.[1]);
            ...
            if (!fs.existsSync(imagePath)) {
                ...
                if (err) {
                    await this.download_image(imageUrl, imagePath).catch(async () => {
                        ...
                        write_log(`[chapter download]下载图片失败 ${this.name} ${chapter.name} ${imageName} ${imageUrl}, 请手动检查. `);
                    })
                }
            }
        }
    ...
    async get_meta() {
        ...
        */
        await this.page_open();
        await this.page.goto(`https://omegascans.org/series/${manga.series_slug}`, {
            waitUntil: 'domcontentloaded',
        ...
        const tagsHtml = mangaPageHtml.match(/<div class=\"flex flex-row flex-wrap gap-2\">([\s\S]*?)<\/div>/s)?.[0];
    ...
    async download_image(url: string, path: string) {
        ...
        const [res, error] = await this.page.goto(url, {
