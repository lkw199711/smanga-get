...
class ToZip {
  ...
  async start() {
    const topToonList = this.getMangaList(this.mangaFloder)
    for (const topToon of topToonList) {
      const fileName = path.basename(topToon)
...
async function zipAndRemoveFolders(sourceDir: string, outputPath: string) {
  try {
    ...
    for (const item of items) {
      // 文件直接复制
      if (!item.isDirectory() && !/.zip$/.test(item.name)) {
